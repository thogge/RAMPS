...
cube_dir = root_dir+"cubes/"
multifield_dir = cube_dir+"multi-field/"
template_files_dir = cube_dir+"/L23/field/tiles/"
...
def main():
    ...
    """
    List to record fields that have already been processed. Add 
    any fields here that should be skipped.
    """
    completed_fields = []
    ...
    freq_to_trans_dict = {"23694":"NH3_1-1","23722":"NH3_2-2",
                          "23870":"NH3_3-3","24139":"NH3_4-4",
                          "24532":"NH3_5-5","22235":"H2O",
    ...
    fields = np.concatenate((np.arange(32)+10,
                             np.array([43,45,47]))).astype(str)

    for field in fields:
        #Define the subdirectory structure
        field_basedir = cube_dir+ "L" + field
        final_dir = field_basedir + "/field/"
        combine_dir = field_basedir + "/field/tiles/"
        
        #Check whether the data in this field has already been processed
        if "L"+field in completed_fields:
            continue
        else: 
            files_exist = []
            for freq in freq_list:
                if (check_files_exist(combine_dir,field,
                                      freq_to_trans_dict[freq],
                                      ["cube","mom0","mom1","rms"]) 
                    or check_files_exist(final_dir,field,
                                         freq_to_trans_dict[freq],
                                         ["cube","mom0","mom1","rms"]))
                    and not overwrite:
        ...
        for i,freq in enumerate(freq_list):
            ...
            freq_file_list = [s for s in all_files if freq+
                              "_MHz_line.fits" in s]
            ...
            if not os.path.exists(combine_file) or overwrite:
            ...
            if not os.path.exists(crop_file) or overwrite:
                execute_string = "python "+scripts_dir+\
            ...
            if (not check_files_exist(final_dir,field,
                                      freq_to_trans_dict[freq],
                                      ["cube","mom0","mom1"])
                or overwrite):
                execute_string = "python "+scripts_dir+\
            ...
            if not os.path.exists(rms_file) or overwrite:
                execute_string = "python "+scripts_dir+\
            ...
            """
            If all of the files were created successfully, 
        ...
        """
        Move all of the processed data from the combine directory 
        to the multi-field directory for analysis.
        """
        for freq in freq_list:
            move_files(file_dir,field,
                       freq_to_trans_dict[freq],
                       ["cube","mom0","mom1","rms"])
    ...
    """
    if overwrite:
        execute_string = "python"+scripts_dir+"make_all_border_cubes.py -o"
    else:
        execute_string = "python"+scripts_dir+"make_all_border_cubes.py"
    print(execute_string)
    os.system(execute_string)


def check_files_exist(file_dir,field,trans,file_suffixes):
    """
    Check if all of the processed data exist.
...
def move_files(file_dir,field,trans,file_suffixes):
    """
    Move the processed data to multifield_dir for analysis.
    ...
    """
    for suff in file_suffixes:
        execute_string = "mv "+file_dir+\
                         "L"+field+"_"+trans+"_"+suff+".fits "+\
                         multifield_dir
        print(execute_string)
        os.system(execute_string)
